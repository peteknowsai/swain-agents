...
swain user outreach-pause <userId> --reason="<verbatim captain quote>" --json
swain user outreach-resume <userId> --json
```

- **engagement** returns last-active, message counts, and both pause states (`paused` for content generation, `outreachPaused` for unsolicited messaging). Check this before any cron-triggered outbound.
- **outreach-pause** mutes unsolicited outbound when the captain asks to stop. Pass their exact words as `--reason` — it shows up in support tooling. Replies to captain-initiated messages are always allowed; this gate only blocks things you initiate.
- **outreach-resume** re-enables unsolicited outbound. Only call this when the captain has explicitly said they want messages again.
